 ...
 * per-site {@link DocSettings} branding. Narrow single-column layout suited to
 * an 80mm thermal slip; amounts are in PHP (no glyph, to stay font-safe).
 */
@Service
...
public class SaleReceiptPdfService {
    ...
    public byte[] generate(String salesNumber) throws IOException {
        ...
        Font h = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 11, Color.BLACK);
        Font small = FontFactory.getFont(FontFactory.HELVETICA, 7, Color.DARK_GRAY);
        Font body = FontFactory.getFont(FontFactory.HELVETICA, 8, Color.BLACK);
        Font bodyB = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 9, Color.BLACK);

        Document doc = new Document(new Rectangle(240f, 640f), 12, 12, 12, 12);
        ...
        try {
            ...
            add(doc, line("Cashier: " + nz(sale.createdBy()), body));
            if (sale.customerName() != null) add(doc, line("Customer: " + sale.customerName(), body));
