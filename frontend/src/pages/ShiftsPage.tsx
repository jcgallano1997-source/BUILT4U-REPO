...
import { ClipboardList, LockKeyhole, PlayCircle } from 'lucide-react'
import { toast } from 'sonner'
import Modal, { btnGhost, btnPrimary, inputCls } from '@/components/Modal'
import {
  closeShift, getCurrentShift, listMyShifts, openShift, peso, posErr,
  type Shift, type ShiftSummary,
} from '@/lib/pos'
  ...
  return (
    <div className="space-y-5">
      <h1 className="flex items-center gap-2 text-xl font-semibold text-slate-800">
        <ClipboardList size={18} className="text-indigo-600" /> Shifts
      </h1>
        ...
        : current === null ? (
          ...
          <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-5">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
              ...
              <Stat label="Maya" value={peso(current.paymayaTotal)} />
              <Stat label="Sales" value={String(current.saleCount)} />
              <Stat label="Opened" value={new Date(current.openedAt).toLocaleString()} />
            </div>
          </div>
        )}
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="rounded-lg bg-white p-3 border border-slate-200">
      ...
      <div className={`text-sm font-medium ${accent ? 'text-indigo-700' : 'text-slate-700'}`}>{value}</div>
    </div>
  )
}
...
function CloseForm({
  ...
  const variance = (Number(counted) || 0) - shift.expectedCash

  async function submit() {
    setSaving(true)
    try {
      await closeShift(shift.shiftNumber, { countedCash: Number(counted || 0), closeNote: note.trim() || undefined })
    ...
    <Modal title={`Close shift ${shift.shiftNumber}`} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div>
          ...
          <input className={inputCls} type="number" value={counted} onChange={(e) => setCounted(e.target.value)} autoFocus />
        </div>
        {counted !== '' && (
